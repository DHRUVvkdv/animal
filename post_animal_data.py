...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("animal_data_poster.log"),
...
# Configuration
API_URL = "https://t35mza5xpmaldiqx63bk4vvpgm0cqxaj.lambda-url.us-east-1.on.aws"  # Replace with your actual API base URL
API_KEY = "dv"  
...
def post_animal_data(cow_id=None, response_type=None):
    ...
    try:
        ...
        logger.info(f"Posting data for {animal_data['cow_id']} with response type: {animal_data['response_type']}")
        
        response = requests.post(
            f"{API_URL}/animal/data",
            headers=headers,
            json=animal_data
        )
        
        if response.status_code in [200, 201]:
            logger.info(f"Successfully posted data for {animal_data['cow_id']}: {response.text[:100]}...")
            return True
        else:
            logger.error(f"Failed to post data: Status {response.status_code}, Response: {response.text}")
            return False
...
def run_interval_posting(cow_id, response_type, interval):
    ...
    """
    logger.info(f"Starting interval posting with cow_id={cow_id}, response_type={response_type}, interval={interval}s")
...
def main():
    ...
    parser.add_argument('--response', choices=RESPONSE_TYPES, help='Response type to use (optimistic or pessimistic)')
    parser.add_argument('--interval', type=int, default=INTERVAL_SECONDS, help=f'Interval between posts in seconds (default: {INTERVAL_SECONDS})')
    parser.add_argument('--once', action='store_true', help='Post once and exit (for testing)')
